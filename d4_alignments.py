...
def clustalw(file_content, query_name):
    ...
    if len(query_pos) == 0:
        ...
        raise ValueError("Protein '{}' name is not unique - can't be used"\
...
def athreem_fasta(data, query_name):
    ...
    if query_pos is None:
        raise ValueError("Query '{}' not present in the alignment and can't be "\
...
def calc_conservation(sequences, query_pos):
    ...
    print("Reduced size from {} to {} unique sequences for conservation "\
...
def alignment_table(alignment_path, query_name):
    ...
    if "CLUSTAL" in data[0].upper():
        ...
        seq, qup = None, None
        raise ValueError("Invalid input format - please use ClustalW, A3M or"\
        "fasta as format")
...
if __name__ == "__main__":
    alignment_table_ = alignment_table("~//Downloads/uni_tubingen_algn/hhmer_clustalw.txt", "pab1")
